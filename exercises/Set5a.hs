...
totalPrice (MkShoppingEntry _ q p) = q * fromIntegral p
...
buyOneMore (MkShoppingEntry t p q) = MkShoppingEntry t p (q+1)
...
study (NthYear y) = if y == 7 then Graduated else NthYear (y+1)
...
rgb Green = [0,1,0]
rgb Blue = [0,0,1]
rgb (Mix a b) = map (/2) $ zipWith (+) (rgb a) (rgb b)
...
toNat :: Int -> Maybe Nat
toNat z = if z < 0 then Nothing else Just (mkNat z)
  where mkNat :: Int -> Nat
        mkNat 0 = Zero
        mkNat n = PlusOne (mkNat (n-1))
------------------------------------------------------------------------------
...
fromBin :: Bin -> Int
fromBin b = go b 1 0
  where go :: Bin -> Int -> Int -> Int
        go End _ acc = acc
        go (O b) n acc = go b (n*2) acc
        go (I b) n acc = go b (n*2) (acc + n)

toBin :: Int -> Bin
toBin 0 = O End
toBin n = foldr f End (toBinary n)
  where toBinary :: Int -> [Int]
        toBinary 0 = []
        toBinary n = r : toBinary q
          where (q,r) = divMod n 2
        f :: Int -> Bin -> Bin
        f n b = if n == 0 then O b else I b
