...
distinct l = length (nub l) == length l
...
longest = foldr compare []
  where compare :: (Ord a) => [a] -> [a] -> [a]
        compare a b 
          | length a > length b = a
...
incrementKey _ [] = []
incrementKey key ((k,v):kvs) = if key == k then (k,v+1) : incrementKey key kvs else (k,v) : incrementKey key kvs
...
winner scores player1 player2 = if lu player2 > lu player1 then player2 else player1
  where lu :: String -> Int 
        lu p = Map.findWithDefault 0 p scores
...
freqs = foldr add Map.empty
  where add :: (Ord a) => a -> Map.Map a Int -> Map.Map a Int
        add x m = case Map.lookup x m of 
...
transfer from to amount bank
  ...
  | otherwise = Map.adjust (+ amount) to (Map.adjust (\a -> a - amount) from bank)
...
maxIndex arr = fst (foldr f a1 as)
  where (a1:as) = assocs arr
        f :: Ord a => (i, a) -> (i, a) -> (i,a)
        f (a,b) (x,y) = if b > y then (a,b) else (x,y)