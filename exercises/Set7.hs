...
add e (Set s) = if member e (Set s) then Set s else Set (sort (e:s))
...
data State = Start | Error | Finished | HasEggs | HasFlour | HasSugar | HasSugarAndFlour | Mixed
  deriving (Eq,Show)
...
step HasFlour AddSugar = HasSugarAndFlour
step HasSugar AddFlour = HasSugarAndFlour
step HasSugarAndFlour Mix = Mixed
...
instance (Eq a, Ord a) => Semigroup (Set a) where
  (Set x) <> (Set y) = Set (sort (union x y))
...
passwordAllowed pw (ContainsSome l) = any (`elem` l) pw
passwordAllowed pw (DoesNotContain l) = not $ any (`elem` l) pw
-- passwordAllowed pw (DoesNotContain l) = not $ passwordAllowed pw (ContainsSome l)
...
operation n a1 a2 = Operation n a1 a2
