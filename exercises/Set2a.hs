...
takeFinal :: Int -> [a] -> [a]
takeFinal n xs = if n > len then xs else take n (drop (len-n) xs) 
  where len = length xs
...
updateAt i e (x:xs) = x : updateAt (i-1) e xs
...
palindromify s = if isPalindrome s then s else palindromify (drop 1 (take (length s - 1) s))
...
safeIndex xs i = if i > (length xs - 1) || i < 0 then Nothing else Just (xs !! i)
